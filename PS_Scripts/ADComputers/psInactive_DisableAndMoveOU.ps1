...
function fnLocal_MoveOU($inactiveList, $ou){
    foreach ($comp in $inactiveList) {
        ...
        write-host "Old OU: " $comp.DistinguishedName
        ...
        write-host "New OU: " $comp.DistinguishedName
    }
}
...
function fnInactive_DisableAndMoveOU{
    ...
    write-host $90Plus
    $Inactive90 = fnAD_GetInactiveComputers  -startDay 180 -endDay  91
    fnLocal_MoveOU -inactiveList $Inactive90 -ou $90Plus
    fnLocal_DisableComputers($Inactive90)
    
    write-host $60Plus
    $Inactive60 = fnAD_GetInactiveComputers  -startDay 90 -endDay  61
